    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.2fr) minmax(280px,0.8fr)", gap: 16 }}>
        <div style={{ display: "grid", gap: 8 }}>
          {pipelineRows.slice(0, 8).map((project) => (
            <div key={project.id} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 12, alignItems: "center", borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.03)", padding: "12px 13px" }}>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {snapshot.entities.regulatoryEvents.slice(0, 8).map((event) => (
            <button
              key={event.id}
              type="button"
