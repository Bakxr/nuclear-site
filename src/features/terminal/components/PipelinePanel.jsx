...
export default function PipelinePanel({ isMobileViewport = false }) {
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: isMobileViewport ? "1fr" : "minmax(0,1.1fr) minmax(280px,0.9fr)", gap: 14 }}>
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div className="np-terminal-scroll" style={terminalScrollAreaStyle(360)}>
            {pipelineRows.slice(0, 10).map((project) => (
              <div key={project.id} className="np-terminal-row np-terminal-row--interactive" style={{ ...terminalDataRowStyle(), display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 12, alignItems: "start" }}>
                <button type="button" onClick={() => selectEntity(project)} className="np-terminal-button" style={{ background: "transparent", border: "none", color: "var(--np-terminal-text)", textAlign: "left", cursor: "pointer", padding: 0, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", marginBottom: 4 }}>
                    <div style={{ fontWeight: 700, fontSize: 12.5 }}>{project.name}</div>
                    <span style={terminalTagStyle({ tone: "amber", compact: true })}>{project.status}</span>
                  </div>
                  <div style={{ fontSize: 11, marginBottom: 6, ...terminalMutedStyle() }}>
                    {project.country} // {project.type} // {project.capacityMw} MW{project.targetYear ? ` // ${project.targetYear}` : ""}
                  </div>
                  <div style={{ fontSize: 11.5, lineHeight: 1.6, ...terminalMutedStyle() }}>{project.summary}</div>
                </button>
          ...
          <div className="np-terminal-scroll" style={terminalScrollAreaStyle(360)}>
            {snapshot.entities.regulatoryEvents.slice(0, 10).map((event) => (
              ...
              >
                ...
                <div style={{ fontSize: 12.5, fontWeight: 700, lineHeight: 1.45, marginBottom: 6 }}>{event.title}</div>
                <div style={{ fontSize: 11.5, lineHeight: 1.6, ...terminalMutedStyle() }}>{event.summary}</div>
              </button>
            ))}
          </div>
        </div>
